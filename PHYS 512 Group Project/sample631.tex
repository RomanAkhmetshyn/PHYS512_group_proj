...
\begin{enumerate}
    ...
    Analysis of results:
    For the single-planet transit, we initially fit for T, Rp/Rs, a/Rs, and the limb darkening coefficients. As evidenced by the trace plots, our MCMC converged around the true parameter values:
    ...
    The first three parameters converged quickly—within just 2000 steps. However, we nonetheless used substantial burn-in in order to account for the fact that the limb darkening coefficients were a bit harder to constrain and not as well converged. The convergence of all parameters is shown in the trace plots.
...
The plot titled “ ,” allows us to visualize the strength of this fit, as it is shown that the blue batman lightcurve aligns exactly with the observed lightcurve in black.

One more thing we would like to note is the incorporation of a step size adjustment section in our mcmc chain function. This involved utilizing a variable named ‘adapt_steps,’ which we arbitrarily set to 10 for our run. Every 10 samples we adjusted the step size to improve the acceptance rate of our parameters and chi-squared values. If the acceptance rate was lower than the target rate, we halved the step size so that the parameter space would be explored more thoroughly. Conversely, if the acceptance rate was too large (i.e., above the target) we increased the step size. The inclusion of this block of code made our mcmc dynamic, as it was constantly improving its sampling.  
